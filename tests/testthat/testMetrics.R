...
test_that("separability", {
  expect_equal(separability(repetition_log, doc), 3 / 14)
  expect_equal(separability(repetition_log_modified, doc_modified), 3 / 17)
  expect_equal(separability(repetition_log_empty, doc_empty), 1)
  expect_equal(separability(repetition_log_event, doc_event), 1)
  expect_equal(separability(repetition_log_simple, doc_simple), 1.5)
})
...
context("cyclicity")
test_that("cyclicity", {
  expect_equal(cyclicity(repetition_log, doc), 1 / 4)
  expect_equal(cyclicity(repetition_log_modified, doc_modified), 4 / 19)
  expect_equal(cyclicity(repetition_log_empty, doc_empty), NaN)
  expect_equal(cyclicity(repetition_log_event, doc_event), 0)
  expect_equal(cyclicity(repetition_log_simple, doc_simple), 0)
})

context("diameter")
test_that("diameter", {
  expect_equal(diameter(repetition_log), 16)
  expect_equal(diameter(repetition_log_modified), 18)
  expect_equal(diameter(repetition_log_empty), 0)
  expect_equal(diameter(repetition_log_event), 1)
  expect_equal(diameter(repetition_log_simple), 4)
})

context("structuredness")
test_that("structuredness", {
  expect_equal(structuredness(repetition_log, doc), 7 / 8)
  expect_equal(structuredness(repetition_log_modified, doc_modified), 17 / 19)
  expect_equal(structuredness(repetition_log_empty, doc_empty), NaN)
  expect_equal(structuredness(repetition_log_event, doc_event), 1)
  expect_equal(structuredness(repetition_log_event, doc_simple), 1)
})

context("depth")
test_that("depth", {
  expect_equal(depth(repetition_log), 1)
  expect_equal(depth(repetition_log_modified), 2)
  expect_equal(depth(repetition_log_empty), 0)
  expect_equal(depth(repetition_log_event), 0)
  expect_equal(depth(repetition_log_simple), 0)
})
...
test_that("Cross connectivity", {
  expect_equal(round(cross_connectivity(repetition_log, doc), digits = 7), 0.1975309)
  expect_equal(round(cross_connectivity(repetition_log_modified, doc_modified), digits = 7),0.1454408)
  expect_equal(cross_connectivity(repetition_log_empty, doc_empty), 0)
  expect_equal(cross_connectivity(repetition_log_event, doc_event), 0)
  expect_equal(cross_connectivity(repetition_log_simple, doc_simple), 0.5)
})
